...
import { z } from 'zod'
import { Implement, ORPCExceptionFilter, ORPCModule } from '../src'
...
// Contract for testing validation (global pipes)
const testValidationContract = {
  validate: oc.route({
    path: '/validate',
    method: 'POST',
  })
...
//
// EXCEPTION HANDLING: Errors thrown in oRPC handlers now bubble up to NestJS,
// allowing global exception filters to catch and transform them. Only decoding
// errors (malformed requests) are caught and converted to BAD_REQUEST responses.
@Controller()
